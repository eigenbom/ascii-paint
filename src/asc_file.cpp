...
int AscFile::load(char *filename) {
	...
	for(int x = 0; x < width; x++) {
		for(int y = 0; y < height; y++) {
			...
			setChar(x, y, c);
			setCharForeground(x, y, fr, fg, fb);
			setCharBackground(x, y, br, bg, bb);
			setSolid(x, y, solid);
...
int AscFile::save(char *filename) {
	...
	for(int x = 0; x < width; x++) {
		for(int y = 0; y < height; y++) {
			char c = getChar(x, y);
			AscRgb f = getCharForeground(x, y);
			AscRgb b = getCharBackground(x, y);
...
void AscFile::setCharForeground(int x, int y, int r, int g, int b) {
	fore[x * height + y].r = r;
	fore[x * height + y].g = g;
	fore[x * height + y].b = b;
}

void AscFile::setCharForeground(int x, int y, AscRgb f) {
	setCharForeground(x, y, f.r, f.g, f.b);
}

void AscFile::setCharBackground(int x, int y, int r, int g, int b) {
	back[x * height + y].r = r;
	back[x * height + y].g = g;
	back[x * height + y].b = b;
}

void AscFile::setCharBackground(int x, int y, AscRgb b) {
	setCharForeground(x, y, b.r, b.g, b.b);
}
...
AscRgb AscFile::getCharForeground(int x, int y) {
	AscRgb rgb;
...
AscRgb AscFile::getCharBackground(int x, int y) {
	AscRgb rgb;
